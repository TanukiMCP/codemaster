...
class CodemasterError(Exception):
    """
    Base exception class for all Codemaster-related errors.
...
class SessionError(CodemasterError):
...
class TaskError(CodemasterError):
...
class ValidationError(CodemasterError):
...
class CapabilityError(CodemasterError):
...
class CommandError(CodemasterError):
...
class ConfigurationError(CodemasterError):
...
class SessionNotFoundError(CodemasterError):
    """Raised when a session cannot be found."""
    pass


class SessionPersistenceError(CodemasterError):
